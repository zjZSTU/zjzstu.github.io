...
本文讨论了在目标识别中生成可能的目标位置的问题。我们引入了选择性搜索，它结合了穷举搜索和分割的优点。像分割一样，我们使用图像结构来指导采样过程；像穷举搜索一样，我们的目标是捕获所有可能的目标位置。我们不再使用单一的技术来生成可能的目标位置，而是使搜索多样化，并使用各种互补的图像分割来处理尽可能多的图像条件。选择性搜索算法能够得到一组数据驱动的、与类无关的、高质量的位置，在10097个位置产生99%的召回率和0.879的平均最佳重叠。与穷举搜索相比，位置数目的减少使得能够使用更强的机器学习技术和更强的外观模型进行目标识别。在本文中，我们证明了我们的选择性搜索能够使用强大的单词袋模型进行识别。选择性搜索软件已开源
...
最后还有一个更根本的问题。具有非常不同特征的区域，例如毛衣上的脸，只有在确定手上的物体是人之后，才能组合成一个物体。因此，如果没有事先的识别，很难确定一张脸和一件毛衣是一个物体的一部分
...
这导致了与传统方法相反的情况：通过识别对象来进行本地化。最近这种目标识别方法在不到十年的时间里取得了巨大的进步。通过从样本中学习得到外观模型，执行详尽的搜索以检查图像中的每个位置，确保不会遗漏任何潜在的对象位置
...
然而，穷尽式搜索本身有几个缺点。因为搜索每个可能的位置在计算上是不可行的，所以必须通过使用规则网格、固定比例和固定纵横比来减少搜索空间。在大多数情况下可能的位置仍然很多，以至于需要实施其他限制。该方法需要简化分类器结构以快速建立外观模型。此外，均匀采样会产生许多框，很明显它们无法识别具体目标。如何不再盲目地使用穷举搜索进行采样，关键问题是：我们能否通过数据驱动的分析来控制采样？
...
图1：图像区域形成目标的原因多种多样。在（b）中的猫可以用颜色而不是质地来区分。在（c）变色龙可以区别于周围叶子的纹理，而不是颜色。在（d）中，车轮可以是汽车的一部分，因为它们是封闭的，而不是因为它们的质地或颜色相似。因此，要以结构化的方式找到对象，必须使用多种多样的策略。此外，图像本质上是分层的，因为在（a）中找不到完整的桌子、沙拉碗和沙拉勺
...
由于目标可以位于图像中的任何位置和比例，因此很自然地搜索每一个位置。然而，视觉搜索空间巨大，使得穷举式搜索在计算上非常昂贵。这对每个地点的评估成本和/或所考虑的地点数量施加了限制。因此，这些滑动窗口技术大多使用粗搜索网格和固定长宽比，使用弱分类器和经济图像特征，如HOG。此方法通常用作分类器级联中的预选步骤
...
与滑动窗口技术相关的是Felzenszwalb等人非常成功的part-based的目标定位方法。他们的方法还使用线性支持向量机和HOG特征进行穷举搜索。但是，它们搜索目标和目标部件，它们的组合会产生令人印象深刻的目标检测性能
...
Lampert等人提出了利用外观模型来指导搜索。这既减轻了使用规则网格、固定比例和固定纵横比的限制，同时也减少了访问的位置数。这是通过使用分支和绑定技术直接搜索图像中的最佳窗口来完成的。虽然他们使用线性分类器就获得了令人印象深刻的结果，然而即使使用非线性分类器，该方法在实践中仍然需要访问超过100000个窗口每幅图像
...
我们提出了选择性搜索，取代了盲穷举搜索或分枝定界搜索。我们使用底层图像结构来生成对象位置，与所讨论的方法相比，这产生了一个完全独立于类的位置集。此外，由于我们不使用固定的长宽比，我们的方法不仅限于目标，而且应该能够找到"草"和"沙"之类的东西（这也适用于[17]）。最后，我们希望生成更少的位置，这将使问题更容易，因为样本的可变性变得更低。更重要的是，它释放了计算能力，可以用于更强大的机器学习技术和更强大的外观模型
...
Carreira和Sminchisescu[4]以及Endres和Hoiem[9]均建议采用分割法产生一组独立的目标假设。两种方法都生成了多个前景/背景分割，学习预测前景分割是完整目标的可能性，并使用该方法来排序分割。这两种算法都显示了在图像中精确描绘物体的能力，得到了[19]的证实，他们在使用[4]在像素级图像分类方面取得了最好的成果。这两种方法在分割中都很常见，都依赖于单一的强算法来识别好的区域。他们通过使用许多随机初始化的前景和背景种子来获得各种位置。相反，我们通过使用不同的分组标准和不同的表示来显式地处理各种图像条件。这意味着更低的计算投资，因为我们不必投资于单一的最佳分割策略，例如使用优秀但昂贵的轮廓检测器[3]。另外，当我们处理不同的图像条件时，我们希望我们的位置具有更高的一致性。最后，我们的选择性搜索范式表明，最有趣的问题不是我们的区域与[4，9]相比如何，而是它们如何能够互补
...
Gu等人[15]解决了基于物体的各个部分进行精细分割和目标识别的问题。他们首先使用基于Arbelaez等人的分组方法生成一组部分假设[3]。每个零件假设都通过外观和形状特征进行描述。然后，利用物体的各个部分对其进行识别和细致的划分，取得了良好的形状识别效果。在他们的工作中，分割是分层的，并且在所有的尺度上产生分段。然而，它们使用单一的分组策略，其发现部分或目标的能力没有得到评估。在这项工作中，我们使用多种互补策略来处理尽可能多的图像条件。我们在评估中包含了使用[3]生成的位置
...
Alexe等人[2]解决穷举搜索中采样空间大的问题，提出搜索与类无关的任何对象。在他们的方法中，他们在具有明确形状对象的目标窗口上训练分类器（而不是像"草"和"沙"这样的东西）。然后，他们不再进行彻底的搜索，而是随机抽取应用分类器的框。具有最高"对象性"度量的框用作一组对象假设。然后使用该集合可以大大减少由类特定对象检测器计算的窗口数。我们把我们的方法和他们的工作进行比较
...
总而言之，我们的新颖之处如下与穷举搜索[8，12，16，36]不同的是，我们使用分割作为选择性搜索，生成一组与类无关的对象位置。与文献[4,9]中的分割方法不同，我们没有关注最佳分割算法[3]，而是使用多种策略来处理尽可能多的图像条件，从而在可能准确地捕获更多对象的同时，大大降低了计算成本。我们使用自底向上的分组过程来生成好的对象位置，而不是在随机抽样的框上学习目标度量[2]
...
在这一节中，我们详细介绍了目标识别的选择性搜索算法，并提出了各种多样的策略来处理尽可能多的图像条件。选择性搜索算法需要考虑以下设计因素：
...
**捕获全尺度**。对象可以在图像中以任何比例出现。此外，一些对象的边界比其他对象的边界更不清晰。因此，在选择性搜索中，必须考虑所有对象比例，如图2所示。最自然的方法是使用分层算法
...
**快速计算**。选择性搜索的目标是产生一组可能的目标位置，用于实际的目标识别框架。这个集合的创建不应该成为计算瓶颈，因此我们的算法应该相当快
...
由于区域可以产生比像素更丰富的信息，因此我们希望尽可能使用基于区域的特征。为了得到一组理想情况下不跨越多个对象的小起始区域，我们使用了Felzenszwalb和Huttenlocher[13]的快速方法，这种方法[3]非常适合这样的要求
...
对于区域$r_{i}$和区域$r_{j}$之间的相似性$s(r_{i}，r_{j})$，我们希望在它们快速计算的约束下仍有各种互补措施。实际上，这意味着相似性应该基于可以通过层次结构传播的特征，也就是说，当将区域$r_{i}$和$r_{j}$合并到$r_{t}$时，区域$r_{t}$的特征需要在不访问图像像素的情况下从$r_{i}$和$r_{j}$的特征计算出来
...
**互补颜色空间**。我们要考虑不同的场景和照明条件。因此，我们在一系列具有不变性的颜色空间中执行我们的分层分组算法。具体地说，我们用一个不断增加的不变性度来表示下列颜色空间：（1）RGB，（2）强度（灰度图像）I，（3）Lab，（4）归一化RGB的rg通道加上强度，表示为rgI，（5）HSV，（6）归一化RGB，表示为rgb，（7）C[14]是划分强度的对色空间，最后（8）HSV的色调通道H。表1列出了具体的不变性属性
...
$S_{colour}(r_{i}, r_{j})$ 测量颜色相似性。具体来说，对于每个区域，我们使用25个bin获得每个颜色通道的一维颜色直方图，我们发现效果很好。当使用三个颜色通道时，这将导致每个区域的颜色直方图$C_{i}={C_{i}^{1},...,C_{i}^{n}}$，其维度为$n=75$。颜色直方图使用$L_{1}$标准化。相似性通过直方图相交来测量：
...
$S_{texture}(r_{i}, r_{j})$ 测量纹理相似性。我们使用快速的类SIFT测量来表示纹理，因为SIFT本身对材料识别很有效[20]。我们采用八个方向的高斯导数，每个颜色通道使用$σ=1$。对于每个颜色通道的每个方向，我们使用10个bin来提取直方图。当使用三个颜色通道时，这将导致纹理直方图$T{i}={T{i}^{1}，…，T{i}^{n}}$用于维度为$n=240$的每个区域$r{i}$。纹理直方图使用$L_{1}$进行规范化。相似性通过直方图相交来测量：
...
$S_{size}(r_{i}, r_{j})$ 鼓励小区域尽早合并。这迫使$S$中的区域（即尚未合并的区域）在整个算法中具有相似的大小。这是可取的，因为它可以确保在图像的所有部分创建所有比例的对象位置。例如，它防止一个区域一个接一个地吞食所有其他区域，只在这个生长区域的位置产生所有的鳞片，而不是在其他地方。$S_{size}(r_{i}，r_{j})$定义为$r_{i}$和$r_{j}$共同占用的图像的分数：
...
$S_{fill}(r_{i}, r_{j})$ 测量区域$r_{i}$和$r_{j}$的匹配程度。想法是填补空白：如果$r{i}$包含在$r{j}$中，则首先合并这些元素以避免出现任何漏洞是合乎逻辑的。换句话说，如果$r_{i}$和$r_{j}$几乎不接触在一起，那么他们将形成一个奇怪的区域，不应该被合并。为了保持度量的快速性，我们只使用区域和包含框的大小。具体来说，我们将$BB_{ij}$定义为$r_{i}$和$r_{j}$周围的紧边界框。现在$S_{fill}(r_{i},r_{j})$表示$BB_{ij}$中没有被$r_{i}$和$r_{j}$所覆盖的区域占整个图像的比例：
...
**互补起始区域**。第三个多样化战略是改变互补的起始区域。据我们所知，[13]的方法是最快的、公开可用的算法，能够产生高质量的起始位置。我们找不到任何其他算法具有类似的计算效率，所以本文只使用这种过度分割。但是请注意，不同的起始区域（已经）是通过改变颜色空间获得的，每个颜色空间具有不同的不变性。此外，我们改变了[13]中的阈值参数$k$
...
在本文中，我们结合了我们的分层分组算法的几种变体的目标假设。理想情况下，我们希望对目标假设进行排序，以便最有可能是对象的位置排在第一位。这使得人们能够根据后续特征提取和分类方法的计算效率，在结果目标假设集的质量和数量之间找到一个很好的折衷
...
根据假设在每个单独分组策略中的生成顺序排列组合对象假设集。然而，当我们将多达80种不同策略的结果结合起来时，这种顺序将过于强调大区域。为了防止这种情况，我们将一些随机性包括在下面。给定一个分组策略$j$，让$r_{i}^{j}$成为在层次结构中的第$i$个位置创建的区域，其中$i=1$表示层次结构的顶部（其对应的区域覆盖整个图像）。我们现在将位置值$v_{i}^{j}$计算为$RND\times i$，其中$RND$是[0,1]范围内的随机数。最后的排名是通过对区域按$v_{i}^{j}$排序获得的
...
**多样化策略**。使用各种颜色空间、相似性度量和初始区域阈值进行实验，所有这些都在第3.2节中详细说明。寻求在生成的对象假设的数量、计算时间和对象位置的质量之间的权衡。我们是根据边界框来做的。这导致了互补技术的选择，这些技术共同作为我们的最终选择搜索方法
...
**目标识别**。我们在第4节详述的目标识别框架中使用选择性搜索得到的位置。在Pascal VOC检测挑战评估性能
...
**位置质量的上界**。研究目标识别框架在使用"完美"质量的对象假设集时的性能。这与选择性搜索生成的位置相比如何？
...
为了评估目标假设的质量，定义了平均最佳重叠（ABO）和平均最佳重叠（MABO）分数，这稍微概括了[9]中使用的测量方法。为了计算特定类$c$的平均最佳重叠，我们计算每个正确标注注释$g_{i}^{c}∈G^{c}$与为相应图像生成的对象假设$L$之间的最佳重叠，并且平均：
...
与平均精度和平均精度均值类似，平均最佳重叠现在定义为所有类的平均ABO
...
其他的工作经常使用来自Pascal重叠标准来测量框的质量[1，16，34]。当上式的重叠度大于0.5时，该准则认为找到了一个物体。然而，在我们的许多实验中，对于大多数类，我们获得了95%到100%的召回率，这使得本文对这个度量过于敏感。然而，在与其他工作进行比较时，我们确实报告了这一措施
...
为了得到高质量的目标位置集合，我们使用多种互补的分组策略来组合目标位置假设。由于对最佳组合的完全搜索在计算上是昂贵的，因此我们仅使用MABO得分作为优化准则来执行贪婪搜索。我们早些时候观察到，这个分数代表了地点数量和质量之间的权衡
...
根据得到的排序，我们创建了三种配置：单一最佳策略、快速选择性搜索和使用单个组件的所有组合（即颜色空间、相似性、阈值）的质量选择性搜索，如表4所示。贪婪搜索强调相似性度量组合的变化。这证实了我们的多样化假设：在质量版本中，在所有相似性的组合旁边，填充和大小是分开的。本文的其余部分使用表4中的三种策略
...
表4：贪婪搜索产生的选择性搜索方法。我们采用所选择的各个多样化策略的所有组合，得到我们的分层分组算法的1、8和80个变体。平均最佳重叠（MABO）分数随着窗口数的增加而稳步上升
...
表5：Pascal 2007测试集上各种方法的召回率、平均最佳重叠（MABO）和窗口位置数的比较
...
图5：目标的定位示例，其最佳重叠分数约为我们的平均最佳重叠0.879。绿色的框是正确标注。红色框是使用"质量"选择性搜索创建的
...
如果我们与其他算法相比，第二高的召回率是0.940，并且是通过跳跃窗口[34]来实现的，每个类使用10000个框。由于我们没有确切的框，我们无法获得MABO分数。接下来是对[12]的详尽搜索，在每个类100352个框（这个数字是所有类的平均值）时，召回率为0.933，MABO为0.829。这明显低于我们的方法，同时使用至少10个以上的对象位置的因素
...
图4探讨了对象假设的质量和数量之间的权衡。在召回方面，我们的“快速”方法优于所有其他方法。[16]的方法对于他们使用的200个位置来说似乎很有竞争力，但是在他们的方法中，框的数量是每个类的，而对于我们的方法，相同的框用于所有类。就MABO而言，[4]和[9]的对象假设生成方法对于生成的每幅图像多达790个对象框位置，都具有良好的数量/质量权衡。然而，这些算法的计算成本是我们的“快速”方法的114倍和59倍
...
有趣的是，[2]的“对象”方法在召回方面表现得很好，但在MABO方面表现得更差。这最有可能是由它们的非最大抑制引起的，这抑制了具有超过0.5的重叠分数的窗口与现有的、更高等级的窗口。当0.5的重叠分数是寻找目标的定义时，这显著地改善了结果，但是对于寻找最高质量位置的一般问题，这一策略效率较低，甚至通过消除更好的位置而有害
...
图6显示了几个方法中每个类的平均最佳重叠。结果表明，对[12]的穷举搜索使用了10倍以上的特定于类的位置，其性能与我们对自行车、桌子、椅子和沙发类的方法相似，对于其他类，我们的方法产生最佳分数。通常，得分最高的类是cat、dog、horse和sofa，这在很大程度上很容易，因为数据集中的实例往往很大。得分最低的班级是瓶类、人类和植物类，这类课程很难，因为实例往往很小。然而，牛、羊和电视都不比人大，而且我们的算法也能很好地找到它们
...
图6：Pascal VOC 2007测试中生成基于框的对象位置的几种方法的每个类的平均最佳重叠分数。对于除了表以外的所有类，我们的“质量”选择性搜索会产生最佳位置。对于20个类中的12个，我们的“快速”选择性搜索优于昂贵的[4，9]。我们总是胜过[2]
...
总而言之，选择性搜索在使用有限数量的框来寻找高质量的对象假设集方面非常有效，其中质量在对象类上是合理一致的。[4]和[9]的方法对于多达790个目标位置具有类似的质量/数量权衡。但是，它们在对象类上有更多的变化。此外，对于我们的“快速”和“高质量”的选择性搜索方法来说，它们的计算成本分别高出59倍和13倍，这是当前对象识别数据集大小的一个问题。一般来说，我们的结论是，选择搜索在0.879mabo处产生最佳质量的位置，同时使用10097个类无关的对象位置的合理数目
...
在本节中，我们将检查选择性搜索生成的区域捕获对象位置的效果。我们在Pascal VOC 2007测试集的分割部分执行此操作。我们比较了[3]的分割和[4，9]的对象假设区域。结果如表6所示。请注意，区域数大于框数，因为几乎没有完全重复的区域
...
表6：Pascal 2007测试的分割部分，根据区域比较找到一组好的潜在目标位置的算法
...
两个[4，9]的目标区域的质量与我们的“快速”选择搜索相似，分别为0.665和0.679 MABO，其中我们的“快速”搜索产生0.666 MABO。虽然[4，9]使用较少的区域，但这些算法的计算成本分别是前者的114倍和59倍。我们的“质量”选择性搜索产生22491个区域，分别比[4,9]快25和13倍，目前最高得分为0.730 MABO
...
图7显示了每个类区域的平均最佳重叠。对于除自行车以外的所有类别，我们的选择性搜索始终具有相对较高的ABO分数。由于自行车是一个线框物体，因此很难准确地描绘出来，因此自行车在区域位置而不是物体位置上的性能要低得多
...
如果我们将我们的方法与其他方法进行比较，那么[9]的方法更适合训练，对于其他类别，我们的“质量”方法可以得到相似或更好的分数。鸟、船、公共汽车、椅子、人、植物和电视的得分为0.05 ABO更好。对于汽车，我们获得了0.12更高的ABO，对于瓶子，甚至0.17更高的ABO。通过观察表6中ABO得分的变化，我们发现选择性搜索的变化比其他方法略低：0.093mabo表示“质量”，0.108表示[9]。然而，这个分数是有偏差的，因为铁丝网框自行车：没有自行车的差异变得更加明显。“质量”选择性搜索的标准差变为0.058，而[9]的标准差变为0.100。再次，这表明依靠多种互补策略而不是单一策略可以产生更稳定的结果
...
图8显示了我们的方法和[4，9]中的几个示例分段。在第一张图片中，其他方法无法将瓶子和书本的白色标签分开。在我们的例子中，我们的策略之一是忽略颜色，而“填充”相似性（公式5）有助于将瓶子和标签组合在一起。缺少的瓶子部分，这是灰尘，已经合并到表之前，这个瓶子段形成，因此“填充”将不起作用。第二幅图像是一幅深色图像的例子，由于使用了多种颜色空间，我们的算法通常有很强的效果。在这个特殊的图像中，部分强度不变的实验室颜色空间有助于隔离汽车。由于我们没有使用[3]中的轮廓检测方法，我们的方法有时会生成具有不规则边界的线段，如猫的第三幅图像所示最后的图像显示了一个非常困难的例子，只有[4]提供了一个精确的片段
...
现在，由于选择性搜索的本质，而不是将方法相互对立，所以更有趣的是看看它们如何能够相互补充。由于[4,9]都有一个非常不同的算法，根据我们的多样化假设，这种组合应该证明是有效的。事实上，如表6的下半部分所示，结合我们的“快速”选择性搜索，在6438个位置找到0.737 MABO。这是一个更高的MABO使用较少的位置比我们的“质量”选择搜索。结合[4，9]和我们的“质量”抽样，在25355个地点得到0.758mabo。这是一个很好的增长，只有少量额外的地点
...
综上所述，选择性搜索对于根据区域生成目标位置是非常有效的。多种策略的使用使得它对各种图像条件和对象类都具有鲁棒性。将[4]、[9]和我们的分组算法组合成一个单一的选择性搜索显示出有希望的改进。考虑到这些改进，以及有更多不同的分区算法可用于选择性搜索，看看我们的选择性搜索范式在计算效率、对象位置数量和对象位置质量方面还能走多远将是一件有趣的事情
...
在本节中，我们将使用Pascal VOC 2010检测任务评估用于对象识别的选择性搜索策略
...
我们的选择性搜索策略能够使用昂贵而强大的图像表示和机器学习技术。在这一部分中，我们使用了第4部分描述的基于单词包的对象识别框架中的选择性搜索。与穷举搜索相比，对象位置的减少使得使用这样一个强大的词包实现成为可能
...
给出计算要求的指示：三个SIFT变体的像素级提取加上视觉单词分配大约需要10秒，并且每个图像执行一次。最后一轮的支持向量机学习在GPU上花费大约8个小时，大约30000个训练示例（33），这是由两个对PASCAL VOC 2010的挖掘负面结果造成的。挖掘硬底片是并行进行的，一轮需要10台机器大约11个小时，每张图片大约40秒。这被分为30秒用于计算可视单词频率，0.5秒用于分类。测试需要40秒来提取特征、可视化单词分配和统计可视化单词的频率，之后每个类需要0.5秒来进行分类。为了进行比较，[12]的代码（没有级联，就像我们的版本一样）需要对每个类的每个图像进行不到4秒的测试。对于20个Pascal类，这使我们的框架在测试期间更快

>We evaluate results using the official evaluation server. This evaluation is independent as the test data has not been released. We compare with the top-4 of the competition. Note that while all methods in the top-4 are based on an exhaustive search using variations on part-based model of [12] with HOG-features, our method differs substantially by using selective search and Bag-of-Words features. Results are shown in Table 7.

我们使用官方评估服务器评估结果。此评估是独立的，因为测试数据尚未发布。我们和比赛的前4名进行比较。请注意，虽然排名前4位的所有方法都是基于使用具有HOG特征的[12]的基于零件的模型的变体的穷举搜索，但是我们的方法通过使用选择性搜索和单词包特征而有本质的不同结果见表7
...
表7:Pascal VOC 2010检测任务测试集的结果。我们的方法是唯一一个基于词包的目标识别系统。它有9个最好的分数，大多是非刚性物体类别，其中差异高达0.056 AP。其他方法基于基于部分的HOG特征，并且在大多数刚性对象类上执行得更好
...
结果表明，该方法对平面类、猫类、牛类、桌子类、狗类、植物类、羊类、沙发类、电视类的学习效果最好。除了桌子、沙发和电视，这些类都是非硬性的。这是意料之中的，因为理论上，Bag of Words比HOG特性更适合这些类。事实上，对于刚性类自行车、瓶子、公共汽车、汽车、人和火车，基于HOG的方法表现得更好。除了tv是个意外。这可能是因为我们的选择性搜索在定位电视上表现良好，见图6
...
在2011年帕斯卡挑战赛中，有几个参赛者的得分明显高于我们的参赛者。这些方法使用词包作为基于部分模型发现的位置的附加信息，从而获得更好的检测精度。然而，有趣的是，通过使用单词包来检测位置，我们的方法对许多类实现了更高的总召回率[10]
...
最后，我们的选择性搜索使我们能够参与ImageNet大规模视觉识别挑战2011（ILSVRC2011）的检测任务，如表8所示。该数据集包含1229413个训练图像和100000个具有1000个不同对象类别的测试图像。测试可以加速，因为从选择性搜索位置提取的特征可以对所有类重用。例如，使用[30]的快速单词包框架，提取SIFT描述符和两个颜色变体所需的时间为6.7秒，而指定给可视单词所需的时间为1.7秒。使用1x1、2x2和3x3空间金字塔分割，需要14秒才能获得所有172032维特征。在金字塔级别上进行级联分类，每个类需要0.3秒。对于1000个类，整个过程需要323秒每个映像进行测试。相反，使用[12]的基于部件的框架，每个类每个图像需要3.9秒，因此每个图像需要3900秒进行测试。这清楚地表明，减少的位置数量有助于向更多类扩展
...
因为Pacal VOC 2012是最新的，也许是最终的VOC数据集，我们简要介绍了这个数据集的结果，以便于与我们未来的工作进行比较。我们使用TRAIN+VAL集呈现框的质量，TRAIN+VAL的分段部分的质量，以及使用官方评估服务器在测试集上使用1x1、2x2、3x3和4x4的空间金字塔的本地化框架
...
在这个实验中，我们研究了我们的选择性搜索位置与最佳位置的接近程度，以确定袋状文字特征的识别精度。我们在帕斯卡VOC 2007测试集上做这个
...
图9中的红线显示了使用“质量”选择搜索方法的前n个框时，我们的对象识别系统的地图分数。性能从0.283 MAP开始，使用MABO为0.758的前500个对象位置。使用前3000个目标位置（MABO为0.855）快速增加到0.356地图，然后使用所有10097个目标位置（MABO为0.883）以0.360地图结束
...
图9：对象识别框架中框选择的理论上限。红色曲线表示使用我们的“质量”选择搜索方法的前n个位置的性能，该方法在500个位置的MABO为0.758，在3000个位置的MABO为0.855，在10000个位置的MABO为0.883。洋红色曲线表示使用相同的前n个位置的性能，但现在结合了地面真实度，这是位置质量的上限（MABO=1）。在10000个位置，我们的目标假设集在目标识别精度方面接近最优
...
洋红线显示了我们的对象识别系统的性能，如果我们包括我们的假设集的基本真实对象的位置，代表了一个对象假设集的“完美”质量的MABO分数为1。当只使用地面真值盒时，可以得到0.592的地图，这是我们的目标识别系统的上限。然而，这一得分迅速下降到0.437地图使用的每幅图像只有500个位置。值得注意的是，当使用所有10079个盒子时，性能下降到0.377 MAP，仅比不包括地面真相时多0.017 MAP。这表明，在10000个目标位置，我们的假设集接近于我们的识别框架所能达到的最佳效果。最有可能的解释是我们使用了SIFT，它被设计为移位不变量[21]。这就导致了图5所示质量的近似框仍然足够好。然而，10000个盒子的“完美”对象假设集和我们的假设集之间的小差距表明，我们到达了这样一个点：对一袋文字的不变性程度可能产生不利影响，而不是有利影响
...
随着盒子数量的增加，“完美”假设集的减少是由于问题难度的增加：盒子越多意味着可变性越高，这使得对象识别问题更加困难。早先我们假设一个详尽的搜索检查图像中所有可能的位置，这使得对象识别问题变得困难。为了测试选择性搜索是否缓解了这个问题，我们还使用了我们的单词包对象识别系统，使用了[12]中的位置进行穷举搜索。这将得到0.336的映射，而MABO为0.829，每个类的对象位置数为100000同样的MABO通过2000个位置的选择性搜索得到。在2000个位置，目标识别精度为0.347。这表明，通过减少位置的可能变化，选择性搜索确实比穷举搜索更容易解决问题
...
总之，目标假设的质与量与目标识别的准确性之间存在着一种权衡关系。高质量的物体位置首先是识别物体所必需的。能够在不牺牲质量的情况下对较少的对象假设进行采样，使得分类问题变得更容易，并有助于改进结果。值得注意的是，在10000个合理的位置上，我们的目标假设集接近于我们的单词袋识别系统的最优值。这表明我们的地理位置是如此的高质量，以至于现在需要比通常在单词包中找到的具有更高辨别能力的特征
...
本文提出了一种适用于选择性搜索的分割方法。我们观察到，图像本身是层次性的，一个区域形成一个对象有很多原因。因此，单一的自底向上分组算法永远无法捕获所有可能的对象位置。为了解决这个问题，我们引入了选择性搜索，其中的主要观点是使用一组不同的互补和分层的分组策略。这使得选择性搜索稳定、健壮，并且独立于对象类，其中对象类型从刚性（例如car）到非刚性（例如cat），理论上也从非晶态（例如water）不等
...
在目标窗口方面，我们的算法优于[2]中的“目标性”，在2134个位置，我们的快速选择性搜索达到0.804的平均最佳重叠质量。与[ 4, 9 ]相比，我们的算法在生成窗口的质量和数量上有相似的权衡，最多790个位置，大约0.790个窗口，它们生成的最大值。然而我们的算法是13-59倍的速度。此外，它可以为每个图像创建多达10097个位置，产生高达0.879的MABO
...
最后，我们证明了选择性搜索可以成功地用于创建一个良好的基于词包的定位和识别系统。事实上，我们已经证明了我们的选择性搜索位置的质量对于我们的基于词包的目标识别来说是接近最优的