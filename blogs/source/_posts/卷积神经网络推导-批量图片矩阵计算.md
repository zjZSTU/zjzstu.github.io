...
因为$a^{(3)}\in R^{(N\cdot 100)\times 150}$，$output^{(2)}\in R^{N\times 6\times 14\times 14}$，$z^{(2)}\in R^{(N\cdot 1176)}$，卷积层$C3$滤波器空间尺寸为$5\times 5$，步长为$1$，所以按照采样顺序将$a^{(2)}$梯度重置回$output^{(2)}$梯度，再重置为$z^{(2)}$梯度
